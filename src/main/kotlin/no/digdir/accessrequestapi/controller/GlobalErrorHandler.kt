...
class GlobalErrorHandler {
    ...
    fun handleNotFound(exception: HttpClientErrorException): ResponseEntity<String> {
        return ResponseEntity.status(exception.statusCode).body(exception.message)
    }

    @ExceptionHandler(HttpServerErrorException::class)
    fun handleNotFound(exception: HttpServerErrorException): ResponseEntity<String> {
